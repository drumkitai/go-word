...
// TestTableDynamicRowMergeValidation
func TestTableDynamicRowMergeValidation(t *testing.T) {
	document.SetGlobalLevel(document.LogLevelInfo)

	t.Run("verify that merging cells after dynamically adding the 29th row does not cause misalignment", func(t *testing.T) {
		doc := document.New()

		// create a 28 row table (as described in the issue)
		...
		t.Logf("original table grid column count: %d", originalGridColCount)

		// verify initial state
		if table.GetRowCount() != 28 {
			t.Fatalf("initial row count is incorrect: expected 28, actual %d", table.GetRowCount())
		}
		if table.GetColumnCount() != 5 {
			t.Fatalf("initial column count is incorrect: expected 5, actual %d", table.GetColumnCount())
		}

		// dynamically add the 29th row
		err = table.AppendRow([]string{"Row29-1", "Row29-2", "Row29-3", "Row29-4", "Row29-5"})
		if err != nil {
			t.Fatalf("add the 29th row failed: %v", err)
		}

		// verify the state after adding the row
		if table.GetRowCount() != 29 {
			t.Fatalf("after adding the row, the row count is incorrect: expected 29, actual %d", table.GetRowCount())
		}

		// verify the number of cells in the 29th row
		row29CellCount := len(table.Rows[28].Cells)
		if row29CellCount != 5 {
			t.Errorf("the number of cells in the 29th row is incorrect: expected 5, actual %d", row29CellCount)
		}

		// verify that the grid column count has not changed
		gridColCountAfterInsert := len(table.Grid.Cols)
		if gridColCountAfterInsert != originalGridColCount {
			t.Errorf("after adding the row, the grid column count has changed: original %d, now %d", originalGridColCount, gridColCountAfterInsert)
		}

		// verify that the number of cells in all rows is 5
