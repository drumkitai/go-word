...
// TestTableBorders tests table border settings
func TestTableBorders(t *testing.T) {
	doc := document.New()
	if doc == nil {
		t.Fatal("failed to create document")
	}
	...
	config := &document.TableConfig{
		...
		Data: [][]string{
			{"Name", "Age", "Occupation"},
			{"Person A", "25", "Engineer"},
			{"Person B", "30", "Designer"},
		},
	}
	...
	if err != nil {
		t.Fatalf("failed to set table borders: %v", err)
	}
