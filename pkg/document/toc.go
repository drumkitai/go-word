// Package document provides Word document table of contents generation functionality
package document
...
// TOCConfig table of contents configuration
...
// TOCField table of contents field
...
// ElementType returns the bookmark end element type
func (b *BookmarkEnd) ElementType() string {
	return "bookmarkEnd"
}

// BookmarkStart bookmark start marker
...
// ElementType returns the bookmark start element type
...
func (d *Document) collectHeadings(maxLevel int) []TOCEntry {
	...
	pageNum := 1 // simplified; actual implementation should calculate real page number
...
// getHeadingLevel gets the heading level of a paragraph
...
// addTOCEntry adds a table of contents entry
	...
	if config.UseHyperlink {
		...
		if config.ShowPageNum {
			if config.DotLeader {
				leaderRun := Run{
					Properties: &RunProperties{},
					Text:       Text{Content: strings.Repeat(".", 20)},
				}
