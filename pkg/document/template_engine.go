...
// TemplateLogger template logger
...
func (tr *TemplateRenderer) logInfo(format string, args ...interface{}) {
	if tr.logger.enabled {
		Infof("[template engine] "+format, args...)
	}
}

func (tr *TemplateRenderer) logError(format string, args ...interface{}) {
	if tr.logger.enabled {
		Errorf("[template engine] "+format, args...)
	}
}
...
// RenderTemplate renders template to a new document
func (tr *TemplateRenderer) RenderTemplate(templateName string, data *TemplateData) (*Document, error) {
	tr.logInfo("starting template render: %s", templateName)
	...
	if err != nil {
		tr.logError("template render failed: %v", err)
		return nil, err
	}
