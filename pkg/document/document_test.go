...
func assertParagraphContent(t *testing.T, paragraphs []*Paragraph, index int, expectedContent string) {
	...
	if index >= len(paragraphs) {
		t.Errorf("paragraph index %d out of range, only %d paragraphs", index, len(paragraphs))
		return
	}

	para := paragraphs[index]
	if len(para.Runs) == 0 {
		t.Errorf("paragraph index %d has no runs", index)
		return
	}

	actualContent := para.Runs[0].Text.Content
	if actualContent != expectedContent {
		t.Errorf("paragraph index %d should have content '%s', got '%s'", index, expectedContent, actualContent)
	}
}
...
func TestAddHeadingParagraph(t *testing.T) {
	...
	testCases := []struct {
		...
		{"first level heading", 1, "Heading1"},
		{"second level heading", 2, "Heading2"},
		{"third level heading", 3, "Heading3"},
		{"ninth level heading", 9, "Heading9"},
	}
...
// TestParagraphSetSpacing
func TestParagraphSetSpacing(t *testing.T) {
	doc := New()
	para := doc.AddParagraph("test spacing")
...
func TestAddPageBreak(t *testing.T) {
	...
	// save and verify document可以正常生成
...
func TestPageBreakAndDeletion(t *testing.T) {
	...
	// verify initial state（2个段落 + 1个分页符 + 1个段落 + 1个分页符 + 1个段落 = 6个元素）
